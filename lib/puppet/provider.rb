...
class Puppet::Provider
  ...
  # @see prefetch
  def self.instances
    raise Puppet::DevError, _("To support listing resources of this type the '%{provider}' provider needs to implement an 'instances' class method returning the current set of resources. We recommend porting your module to the much simpler Resource API instead: https://puppet.com/search/docs?keys=resource+api") % { provider: self.name }
  end
